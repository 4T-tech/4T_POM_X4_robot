...
void Yaw_Correct(uint8_t motion_speed)
{
    ...
    // 动态补偿仅在移动状态有效
    static float yaw_ref = 0.0f;    // 参考航向角
    static uint8_t moving_last = 0; // 上一帧移动状态

    // 运动开始时记录参考航向角
    if (moving_last == 0 && motion_speed > 0) {
        yaw_ref = -LSM6DSOW_Yaw;
        if (yaw_ref < 0) yaw_ref += 360.0f;
        if (yaw_ref >= 360.0f) yaw_ref -= 360.0f;
    }
